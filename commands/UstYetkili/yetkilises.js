...
module.exports = {
  ...
  if (!message.member.permissions.has(8n)) return message.react(red)
    ...
    if (!sec) {
      ...
      let cino = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(ayar.registerPerm)).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
