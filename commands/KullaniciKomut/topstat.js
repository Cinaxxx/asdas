...
${message.guild.name} sunucusunun toplam ses ve chat bilgileri gösterilmektedir.
    `)
    .addFields({ name: `**Genel ses sıralaması**(\`Toplam ${moment.duration(voiceGuildData ? voiceGuildData.topStat : 0).format("H [saat], m [dakika]")}\`)`, value: `${voiceUsers.length > 0 ? voiceUsers : "Veri Bulunmuyor."}`, inline: true })
    .addFields({ name: `**Genel chat sıralaması**(\`Toplam ${Number(messageGuildData ? messageGuildData.topStat : 0).toLocaleString()} mesaj\`)`, value: `${messageUsers.length > 0 ? messageUsers : "Veri Bulunmuyor."}`, inline: true })
