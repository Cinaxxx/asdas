...
module.exports = {
  ...
  run: async (client, message, args, embed, prefix) => {
    ...
    if (!message.member.voice.channel) {
      return message.channel.send({ content: "Bir ses kanalında olmalısın!" });
  }
    if (!member) {
      return message.channel.send({ content: "Bir üye etiketle ve tekrardan dene!" });
  }
    if (!member.voice.channel) {
      return message.channel.send({ content: "Bu kullanıcı herhangi bir ses kanalında bulunmuyor!" });
  }
    if (message.member.voice.channel === member.voice.channel) {
      return message.channel.send({ content: "Zaten aynı kanaldasınız!" });
  }
  ...
  .setDisabled(true),
    ...
    if (message.member.permissions.has("ADMINISTRATOR")) {
      ...
      message.react(green)
      message.channel.send({ embeds: [embed.setThumbnail(message.author.avatarURL({ dynamic: true, size: 2048 })).setDescription(`${message.author}, ${member} kişisini yanınıza taşıdınız.`)] });
      const log = embed
        .setColor("#2f3136")
        .setDescription(`
