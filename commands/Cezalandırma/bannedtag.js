...
module.exports = {
  ...
  run: async (client, message, args, perm, prefix) => {
        ...
        const data = await bannedTag.findOne({ guildID: settings.guildID });
            ...
            if (args[0] == "ekle") {
                if (!args[1]) return message.channel.send({ content:"Yasaklıya atmak istediğin tagı belirtmelisin."})
                if (!data) {
                    let arr = []
                    arr.push(args[1])
                    ...
                    await message.channel.send({ content:"**" + args[1] + "** tagında " + üyeler.size + " kişi bulundu hepsine yasaklı tag permi veriyorum."})
                   await bannedTag.findOneAndUpdate({ guildID: message.guild.id }, { $push: { tags: args[1] } }, { upsert: true });
                    ...
                    if (taglar.includes(args[1])) return message.channel.send({ content:"Yasaklıya atmak istediğin tag veritabanında zaten yasaklı."})
            ...
            if (args[0] == "liste" && !args[1]) {
                if (!data || data && !data.taglar.length) return await message.channel.send({ content:"Sunucuda yasaklanmış tag bulunmamakta."})
                let num = 1
                let arrs = data.taglar.map(x => `\`${num++}.\` ${x} - (${client.users.cache.filter(s => data.taglar.some(ozi => s.tag.includes(ozi))).size} üye)`)
                await message.channel.send({ content: arrs.join("\n") })
            }

            if (args[0] == "liste" && args[1] == "üye") {
                if (!args[2]) await message.channel.send({ content:"Üyelerini listelemek istediğin yasaklı tagı belirtmelisin."})
                if (!data || data && !data.taglar.length) return await message.channel.send({ content:"Veritabanında listelenecek yasaklı tag bulunmuyor."})
                if (!data.taglar.includes(args[2])) return await message.channel.send({ content:"**" + data.taglar.join(",") + "** tag(ları) sunucuda yasaklanmış durumdadır. Belirttiğin tag veritabanında bulunmuyor."})
                let üyeler = message.guild.members.cache.filter(x => {
            ...
            if (args[0] == "kaldır") {
                if (!data || data && !data.taglar.length) return await message.channel.send({ content:"Veritabanında kaldırılılacak yasaklı tag bulunmuyor."})
                if (!data.taglar.includes(args[1])) return await message.channel.send({ content:"Belirttiğin tag yasaklı tag listesinde bulunmuyor"})
                let üyeler = message.guild.members.cache.filter(x => {
                    return x.user.tag.includes(args[1])
                })
                await message.channel.send({ content:"**" + args[1] + "** tagında " + üyeler.size + " kişi bulundu hepsineden yasaklı tag permini alıp sistemden tagı kaldırıyorum."})
            ...
            if (args[0] == "kontrol") {
                if (!data || data && !data.taglar.length) return await message.channel.send({ content:"Veritabanında kontrol edilecek yasaklı tag bulunmuyor."})
