...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react(green)
    message.channel.send({ content:`${green} \`(${ban.user.username.replace(/\`/g, "")} - ${ban.user.id})\` adlı üyenin banı ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
