...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.react(green)
    message.channel.send({content:`${green} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    ...
    .setColor("#ed4245")
    .setDescription(`
${member.toString()} (\`${member.id}\`) üyesi ${time} boyunca ${message.author} tarafından metin kanallarında susturuldu.

• Voice Mute Atılma: <t:${Math.floor(Date.now() / 1000)}> (<t:${Math.floor(Date.now() / 1000)}:R>)
• Voice Mute Bitiş: <t:${Math.floor((Date.now() + duration) / 1000)}> (<t:${Math.floor((Date.now() + duration) / 1000)}:R>)
