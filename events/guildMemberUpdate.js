  ...
  await newMember.guild.fetchAuditLogs({
    ...
    newMember.roles.cache.forEach(async role => {
      if (!oldMember.roles.cache.has(role.id)) {
        const emed = new MessageEmbed()
          ...
          .setColor("RANDOM")
          .setDescription(`Kişinin eklenen ve alınan tüm rollerine bakmak için \`!rollog @Cinax\` komutunu kullanın \n**Rol Eklenen kişi**\n ${hedef} - **${hedef.id}** `)
          .addFields(
          ...
          .setTimestamp()
          client.channels.cache.get(Log.rollogchannel).send({ embeds: [emed]})
        roller.findOne({
          user: hedef.id
        }, async (err, res) => {
          if (!res) {
    ...
    oldMember.roles.cache.forEach(async role => {
      if (!newMember.roles.cache.has(role.id)) {
        ...
        .setAuthor({ name: hedef.tag, iconURL: hedef.displayAvatarURL({ dynamic: true }) })
        .setColor("RANDOM")
          .setDescription(`Kişinin alınan ve eklenen tüm rollerine bakmak için \`!rollog @Cinax\` komutunu kullanın \n**Rolü Alınan kişi** \n${hedef} - **${hedef.id}**`)
          .addFields(
          ...
          .setFooter({ text: yapan.tag, iconURL: yapan.displayAvatarURL({ dynamic: true }) })
          .setTimestamp()
          client.channels.cache.get(Log.rollogchannel).send({ embeds: [emeed]})
        roller.findOne({
          user: hedef.id
        }, async (err, res) => {
          if (!res) {
