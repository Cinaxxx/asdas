...
if (!oldState.channel && newState.channel) return channel.send({ content:`${newState.member.displayName} üyesi \`${newState.channel.name}\` adlı sesli kanala girdi!`});
if (oldState.channel && !newState.channel) return channel.send({ content:`${newState.member.displayName} üyesi \`${oldState.channel.name}\` adlı sesli kanaldan ayrıldı!`});
if (oldState.channel.id && newState.channel.id && oldState.channel.id != newState.channel.id) return channel.send({ content:`${newState.member.displayName} üyesi ses kanalını değiştirdi! (\`${oldState.channel.name}\` => \`${newState.channel.name}\`)`});
...
if (!oldState.serverMute && newState.serverMute) return channel2.send({ embeds: [new MessageEmbed().setColor("RANDOM").setAuthor({ name: client.guilds.cache.get(cfg.guildID).name, iconURL: client.guilds.cache.get(cfg.guildID).iconURL({dynamic:true})}).setFooter({ text: `${moment(Date.now()).format("LLL")}`}).setDescription(`
